...
public class ScoreServiceV6 {
	...
	public void inputScore() {
		for(int i = 0 ; i < 5 ; i ++) {
			...
			for(int sb = 0; sb < subject.length ; sb++) {
				scores[i]
						= inService.inputValue(subject[i], 0, 100);
				if(scores[i] == null) {
					return;
				}
			}
	...
	public void inputScore1() {
		...
		for(int st = 0 ; st < 5 ; st++) {
			...
			Integer intKor
			= inService.inputValue("국어",0,100);
			if(intKor == null) {
				return;
				
				Integer intEng
				= inService.inputValue("영어",0,100);
				if(intEng == null) {
					return;
					
					Integer intMath
					= inService.inputValue("수학",0,100);
					if(intMath == null) {
						return;
			}
					ScoreVO scoreVO = new ScoreVO();
